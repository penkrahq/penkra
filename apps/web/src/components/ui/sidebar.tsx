  ...
  return (
    <SidebarInstanceContext.Provider value={instanceContextValue}>
      ...
      >
        ...
        <div
          className={cn(
            "fixed inset-y-0 z-0 hidden h-svh w-(--sidebar-width) transition-[transform,width,visibility] duration-200 ease-linear motion-reduce:transition-none md:flex",
            side === "left"
...
function SidebarGroupLabel({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "flex h-8 shrink-0 items-center rounded-lg px-2 font-medium text-sidebar-foreground text-[length:var(--app-font-size-ui,12px)] outline-hidden ring-ring/60 transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-1 [&>svg]:size-4 [&>svg]:shrink-0",
...
function SidebarGroupContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("w-full text-[length:calc(var(--app-font-size-base,12px)*1.1667)]", className)}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full cursor-pointer items-center gap-2 overflow-hidden rounded-xl p-2 text-left text-[length:calc(var(--app-font-size-base,12px)*1.1667)] outline-hidden ring-ring/60 transition-[width,height,padding] hover:bg-[var(--sidebar-accent)] focus-visible:ring-1 active:bg-[var(--sidebar-accent-active)] active:text-[var(--sidebar-accent-foreground)] disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pe-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-[var(--sidebar-accent-active)] data-[active=true]:text-[var(--sidebar-accent-foreground)] data-[state=open]:hover:bg-[var(--sidebar-accent)] group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg:not([class*='size-'])]:size-4 [&>svg]:shrink-0",
  {
    ...
    variants: {
      size: {
        default: "h-8 text-[length:calc(var(--app-font-size-base,12px)*1.1667)]",
        lg: "h-12 text-[length:calc(var(--app-font-size-base,12px)*1.1667)] group-data-[collapsible=icon]:p-0!",
        sm: "h-7 text-[length:var(--app-font-size-ui,12px)]",
      },
      variant: {
...
function SidebarMenuBadge({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "pointer-events-none absolute right-1 flex h-5 min-w-5 select-none items-center justify-center rounded-lg px-1 font-medium text-sidebar-foreground text-[length:var(--app-font-size-ui,12px)] tabular-nums",
...
function SidebarMenuSubButton({
  ...
  const defaultProps = {
    className: cn(
      ...
      size === "sm" && "text-[length:var(--app-font-size-ui,12px)]",
      size === "md" && "text-[length:calc(var(--app-font-size-base,12px)*1.1667)]",
      "group-data-[collapsible=icon]:hidden",
      className,
    ),
    "data-active": isActive,
