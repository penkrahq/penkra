...
function readState(threadId: string): import("@penkra/sdk").AppThreadState {
  ...
  const shell = useStore.getState().threadShellById?.[typedThreadId];
  if (!shell) {
    throw Object.assign(new Error("The Thread is unavailable."), {
      code: "THREAD_NOT_FOUND",
    });
  }
  let live: ReturnType<ReturnType<typeof requireDesktopThreadLiveHandlers>["read"]> | null = null;
  try {
    live = requireDesktopThreadLiveHandlers(threadId).read();
  } catch {
    // Background deck members have authoritative shell lifecycle state even when
    // their composer surface is not mounted in this window.
  }
