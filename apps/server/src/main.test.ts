...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Penkra to non-loopback host 0\.0\.0\.0/);
    }),
  );
        ...
        Effect.provideService(ProjectionSnapshotQuery, {
          getSnapshot: () =>
            Effect.succeed({
              ...
              decks: [] as OrchestrationReadModel["decks"],
          ...
          getThreadMessageContext: () => Effect.die("unused"),
          ...
          getThreadTurnsPage: () => Effect.die("unused"),
          getPendingStartOutcome: () => Effect.die("unused"),
        }),
