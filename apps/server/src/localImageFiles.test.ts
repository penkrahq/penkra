...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("penkra-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("penkra-codex-home-");
  ...
  it("allows images written to the PENKRA_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Penkra overlay
    // directory (see resolvePenkraCodexHomeOverlayPath). Generated images therefore
    // live under <PENKRA_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    process.env.PENKRA_HOME = penkraHome;
    try {
      const result = await resolveAllowedLocalPreviewFile({
        requestedPath: imagePath,
        cwd: null,
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/penkra-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "penkra-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("penkra-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("penkra-image-tmp-root-");
